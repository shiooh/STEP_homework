...
typedef struct my_metadata_t {
  size_t size;
  struct my_metadata_t *next_free;
  struct my_metadata_t *left_neighbour;
} my_metadata_t;
...
void my_add_to_free_list(my_metadata_t *metadata) {
  assert(!metadata->next_free);
  int list_index = my_find_list_index(metadata->size);
  metadata->next_free = my_heap.free_head_list[list_index];
  my_heap.free_head_list[list_index] = metadata;
}

void my_remove_from_free_list(my_metadata_t *metadata, my_metadata_t *prev) {
  if (prev) {
    prev->next_free = metadata->next_free;
  } else {
    int list_index = my_find_list_index(metadata->size);
    my_heap.free_head_list[list_index] = metadata->next_free;
  }
...
//
// -------------------- 単方向リスト neighbour の更新 ------------------- //
// - 空き領域, 使用中領域を合わせたすべての領域をアドレス順の!!逆!!に並べる - //
// ------------ dummy ← metadata1 ← metadata2 ... ← NULL -------------- //
// ------------------ ↖ metadata3 ← metadata4 ... ← NULL -------------- //
//

my_metadata_t * calc_right_neighbour(my_metadata_t *metadata){
  my_metadata_t *ptr = metadata + 1;
  my_metadata_t *right_metadata = (my_metadata_t *)((char *)ptr + metadata->size);
  if((uintptr_t)right_metadata % 4096 == 0)return NULL;
  return right_metadata;
}

void my_initialize_neighbour(){
  my_heap.dummy.left_neighbour = NULL;
}

void my_make_new_neighbour_list(my_metadata_t *metadata){
  metadata -> left_neighbour = &my_heap.dummy;
}

void my_insert_neighbour_after(my_metadata_t *new_data, my_metadata_t *cur_data){
  // cur_data ← right_metadata(or NULL)
  // → cur_data ← new_data ← right_metadata
  my_metadata_t *right_metadata = calc_right_neighbour(cur_data);
  if(right_metadata){
      right_metadata->left_neighbour = new_data;
  }
  new_data->left_neighbour = cur_data;
}

void my_remove_from_neighbours(my_metadata_t *metadata){
  // a ← metadata ← b(or NULL)
  // → a ← b(or NULL)
  my_metadata_t *right_metadata = calc_right_neighbour(metadata);
  if(right_metadata){
    right_metadata->left_neighbour = metadata->left_neighbour;
  }
  metadata->left_neighbour = NULL;
}
...
void *my_malloc(size_t size) {
  ...
  for(int i=smallest_list_index; i < my_heap.free_head_list_size; ++i){
    ...
    while (metadata) {
      if (metadata->size >= size &&
        ...
        smallest_prev = prev;
        if(smallest_matadata->size == size)break;
      }
      prev = metadata;
      metadata = metadata->next_free;
    }
  ...
  if (!smallest_matadata) {
    ...
    my_make_new_neighbour_list(metadata);
  ...
  if (remaining_size > sizeof(my_metadata_t)) {
    ...
    smallest_matadata->next_free = NULL;
...
void my_joint_right(my_metadata_t *metadata){
  my_metadata_t *right_metadata = calc_right_neighbour(metadata);
  if (right_metadata != NULL && right_metadata->next_free != NULL) {
...
my_metadata_t * my_joint_left(my_metadata_t *metadata){
  my_metadata_t *left_metadata = metadata->left_neighbour;
  if ((uintptr_t)metadata % 4096 != 0 && left_metadata != &my_heap.dummy && left_metadata->next_free != NULL) {
